...
# GPTについてざっくりとした理解
- 大量のテキストによって学習された無数のパラメータを用いて、入力された文章の次に来る単語を確率的に予測するモデル。シンプルに言えば変数を大量にもつ関数で、確率分布とみなせる出力から次単語を予測する。
- 知能をもつかのような振る舞いを実現している本質的な機構はTransformerBlock。より正確にはその中のMultiHeadAttention。
  - 逆に言えばその他のレイヤー(FeedForwardやLayerNormなど)は、上述した機構の精度を上げるための手段でしかなく、知能に近い振る舞いを持つ機構自体を指すわけではない。
- MultiHeadAttentionは、Query/Key/Valueの3つの仕組みを活用している。QueryとKeyは特定の単語までの
